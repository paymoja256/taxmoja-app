...
def invoice_webhook(request, client_acc_id):
    try:
        ...
        struct_logger.info(
            event="processing dear invoice", request=request, response=mita_invoice
        )

        return HttpResponse(status=200)
    except Exception as ex:
        struct_logger.error(event="processing dear invoice", error=str(ex))
...
def creditnote_webhook(request, client_acc_id):
    try:
        ...
        struct_logger.info(event="processing dear creditnote", response=mita_invoice)

        return HttpResponse(status=200)
    except Exception as ex:
        struct_logger.error(event="processing dear creditnote", error=str(ex))
        return HttpResponse(status=500)
...
def stock_configuration_webhook(request, client_acc_id):
    try:
        ...
        struct_logger.info(
            event="processing dear goods configuration", response=goods_configuration
        )

        return HttpResponse(status=200)
    except Exception as ex:
        struct_logger.error(event="processing dear goods configuration", error=str(ex))
        return HttpResponse(status=500)
...
def stock_adjustment_webhook(request, client_acc_id):
    try:
        ...
        struct_logger.error(event="processing dear goods adjustment", error=str(ex))
        return HttpResponse(status=500)
