...
def xero_invoice_webhook(request, client_acc_id):
    try:
        ...
        client_data = get_client_data(client_acc_id)
...
def create_xero_goods_configuration(good_instance):
    try:
        ...
        struct_logger.info(event='create_xero_goods_configuration', xero_product=item, account=good_instance)
        ...
        efris_response = send_mita_request('stock/configuration', efris_stock_configuration_payload, client_data)
...
def create_xero_goods_adjustment(good_instance):
    try:
        ...
        mita_stock = send_mita_request('stock/adjustment', efris_stock_adjustment_payload, client_data)
        ...
        if xero_invoice['Status'] == "AUTHORISED":
            ...
            try:
                ...
                if credentials.expired():
                    ...
                    client_data.save()
                xero = Xero(credentials)
                credit_note = xero.creditnotes.get(u'{}'.format(xero_credit_note['ID']))
...
def send_mita_request(url_ext, payload, client_account):
    url = "http://127.0.0.1:8080/taxmoja/{}".format(url_ext)
    headers = {
...
def test_json_settings(request):
    from settings_json import get_setting

    return HttpResponse("items retrieved {}".format(get_setting('UG')))


def get_client_data(client_account_id):
    try:

        client_data = XeroCredentials.objects.get(id=client_account_id)
        ...
        raise BadRequest('Invalid request: Can not find client account {}'.format(str(ex)))
