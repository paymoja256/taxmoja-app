    ...
    for xero_invoice in xero_invoices:
        ...
        if xero_invoice["Status"] in ("AUTHORISED", "PAID"):
            try:
                ...
                credentials = xero_client_credentials(client_data)
                xero = Xero(credentials)
                credit_note = xero.creditnotes.get("{}".format(xero_credit_note["ID"]))
                ...
                mita_payload = {
                    "invoice_details": {
                        "invoice_code": xero_invoice["CreditNoteNumber"],
                        "cashier": "Eseza Muwanga",
                        "payment_mode": "107",
                        "currency": xero_invoice["CurrencyCode"],
                        "invoice_type": "1",
                        "invoice_kind": "1",
                        "goods_description": xero_invoice["CreditNoteID"],
                        "industry_code": "",
                        "original_instance_invoice_id": original_invoice[
                            "InvoiceNumber"
                        ],
                    ...
                    "instance_invoice_id": xero_invoice["CreditNoteNumber"],
                }
                print(mita_payload)
